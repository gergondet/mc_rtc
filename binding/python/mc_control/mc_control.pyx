...
cdef class MCController(object):
    ...
    def contacts(self):
        end = self.base.contacts().const_end()
        it = self.base.contacts().const_begin()
        out = []
        while it != end:
            out.append(mc_rbdyn.ContactFromC(<c_mc_rbdyn.Contact &>deref(it)))
            preinc(it)
        return out
    def hasContact(self, mc_rbdyn.Contact c):
        return self.base.hasContact(c.impl)
    def addCollisions(self, r1, r2, colsIn):
        cdef vector[c_mc_rbdyn.CollisionDescription] cols
        if isinstance(r1, unicode):
            r1 = r1.encode(u'ascii')
        if isinstance(r2, unicode):
            r2 = r2.encode(u'ascii')
        for c in colsIn:
            if not isinstance(c, mc_rbdyn.CollisionDescription):
                c = mc_rbdyn.CollisionDescription(*c)
            cols.push_back((<mc_rbdyn.CollisionDescription>c).impl)
        return self.base.addCollisions(r1, r2, cols)
    def removeCollisions(self, r1, r2, colsIn = None):
        cdef vector[c_mc_rbdyn.CollisionDescription] cols
        if isinstance(r1, unicode):
            r1 = r1.encode(u'ascii')
        if isinstance(r2, unicode):
        ...
        for c in colsIn:
            if not isinstance(c, mc_rbdyn.CollisionDescription):
                c = mc_rbdyn.CollisionDescription(*c)
            cols.push_back((<mc_rbdyn.CollisionDescription>c).impl)
        return self.base.removeCollisions(r1, r2, cols)
    ...
    def realRobot(self, name = None):
        if name is None:
            return mc_rbdyn.RobotFromC(self.base.realRobot())
        else:
            if isinstance(name, unicode):
                name = name.encode(u'ascii')
            return mc_rbdyn.RobotFromC(self.base.realRobot(name))
    def realRobots(self):
        return mc_rbdyn.RobotsFromRawPtr(&(self.base.realRobots()))
    def loadRobot(self, mc_rbdyn.RobotModule rm, name, sva.PTransformd posW = sva.PTransformd.Identity()):
        if isinstance(name, unicode):
            name = name.encode(u'ascii')
        return mc_rbdyn.RobotFromC(self.base.loadRobot(rm.impl, name, deref(posW.impl)))
    ...
    def gui(self):
        return mc_rtc_gui.StateBuilderFromRef(self.base.gui())
    property qpsolver:
        def __get__(self):
...
cdef class MCPythonController(MCController):
    ...
    property dynamicsConstraint:
        def __get__(self):
            return mc_solver.DynamicsConstraintFromPtr(self.impl.dynamicsConstraint())
    property kinematicsConstraint:
        def __get__(self):
            return mc_solver.KinematicsConstraintFromPtr(self.impl.kinematicsConstraint())
    property collisionConstraint:
        def __get__(self):
            return mc_solver.CollisionsConstraintFromPtr(self.impl.collisionConstraint())
    property postureTask:
        def __get__(self):
            return mc_tasks.PostureTaskFromPtr(self.impl.postureTask())
