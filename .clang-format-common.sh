...
src_files=`find benchmarks binding/python/include include src tests utils plugins -name '*.cpp' -or -name '*.h' -or -name '*.hpp'|grep -Ev "utils/mc_bin_flatbuffers/"|grep -Ev "include/mc_rtc/robin_hood.h"`
